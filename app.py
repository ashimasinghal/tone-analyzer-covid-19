...
app.layout = html.Div(children=[
  html.H2('Real-time Twitter Sentiment Analysis for Lockdown during COVID-19 Analysis ', style={
      'textAlign': 'center'
  }),
  html.H4('(Last updated: JULY, 2020)', style={
      'textAlign': 'right'
  }),
  ...
  html.Div(
      className='row',
      children=[ 
          dcc.Markdown("__Author's Words__: Dive into the industry and get my hands dirty. That's why I start this self-motivated independent project. If you like it, I would appreciate for starring⭐️ my project on [GitHub]()!✨"),
      ...
      children=[
          ...
          html.Div(
              ...
              children=[
                  ...
                  html.A(
                      'Chulong Li',
                      href='https://www.linkedin.com/in/ashima-singhal-48320516b/'
                  )                    
              ]
          )                                                          
...
def update_graph_live(n):
  ...
  query = "SELECT id_str, text, created_at, polarity, user_location, user_followers_count FROM {}".format(settings2.TABLE_NAME)
  ...
  result = result.rename(columns={"id_str": "Num of '{}' mentions".format(settings2.TRACK_WORDS[0]), "created_at":"Time"})  
  ...
  neu_num = result[result['Time']>min10]["Num of '{}' mentions".format(settings2.TRACK_WORDS[0])][result['polarity']==0].sum()
  neg_num = result[result['Time']>min10]["Num of '{}' mentions".format(settings2.TRACK_WORDS[0])][result['polarity']==-1].sum()
  pos_num = result[result['Time']>min10]["Num of '{}' mentions".format(settings2.TRACK_WORDS[0])][result['polarity']==1].sum()
  ...
  daily_tweets_num = back_up['daily_tweets_num'].iloc[0] + result[-6:-3]["Num of '{}' mentions".format(settings2.TRACK_WORDS[0])].sum()
  ...
  children = [
                      ...
                      dcc.Graph(
                          ...
                          figure={
                              'data': [
                                  go.Scatter(
                                      x=time_series,
                                      y=result["Num of '{}' mentions".format(settings2.TRACK_WORDS[0])][result['polarity']==0].reset_index(drop=True),
                                      name="Neutrals",
                                      opacity=0.8,
                                      mode='lines',
                                  ...
                                  go.Scatter(
                                      x=time_series,
                                      y=result["Num of '{}' mentions".format(settings2.TRACK_WORDS[0])][result['polarity']==-1].reset_index(drop=True).apply(lambda x: -x),
                                      name="Negatives",
                                      opacity=0.8,
                                      mode='lines',
                                  ...
                                  go.Scatter(
                                      x=time_series,
                                      y=result["Num of '{}' mentions".format(settings2.TRACK_WORDS[0])][result['polarity']==1].reset_index(drop=True),
                                      name="Positives",
                                      opacity=0.8,
                                      mode='lines',
              ...
              html.Div(
                  ...
                  children=[
                      ...
                      html.Div(
                          children=[
                              html.P("Currently tracking \"lockdown\" on Twitter in Pacific Daylight Time (PDT).",
                                  style={
                                      'fontSize': 25
                                  }
                              ),
...
def update_graph_bottom_live(n):
  ...
  DATABASE_URL = os.environ['DATABASE_URL']
  conn = psycopg2.connect(DATABASE_URL, sslmode='require')
  query = "SELECT id_str, text, created_at, polarity, user_location FROM {}".format(settings2.TABLE_NAME)
  df = pd.read_sql(query, con=conn)
  conn.close()
...
'Tamil Nadu','TN','Telangana','TG','Tripura','TR','Uttarakhand','UK','Uttar Pradesh','UP','West Bengal','WB']
  ...
  fd['Marker_Color'] = fd['Polarity'].apply(lambda x: 'rgba(255, 50, 50, 0.6)' if x < -0.1 else       ('rgba(184, 247, 212, 0.6)' if x > 0.1 else 'rgba(131, 90, 241, 0.6)'))
  fd['Line_Color'] = fd['Polarity'].apply(lambda x: 'rgba(255, 50, 50, 1)' if x < -0.1 else       ('rgba(184, 247, 212, 1)' if x > 0.1 else 'rgba(131, 90, 241, 1)'))
              ...
              html.Div([
                  dcc.Graph(
                      ...
                      figure = {
                          ...
                          'layout': {
                              'title': "Geographic Segmentation for ASIA",
                              'geo':{'scope':'asia'}
                          }
                      }
                  )
